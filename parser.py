...
raw_data = pd.DataFrame(columns = ['Scout ID', 'Team #', 'Match #', 'Alliance Color', 'On Field?', 
    'Left Tarmac in Auto?', 'Cargo Collected', 'Auto Upper', 'Auto Lower', 'Teleop Upper', 'Teleop Lower',
...
for string in pre_csv:
    ...
    #Skip 19, Opposing Cargo doesn't really exist yet
    broken_drive = string[20] #bool
    played_def = string[21] #bool
    unctrl_crash = string[22] #bool
    
    int_conv = [auto_upper, auto_lower, teleop_upper, teleop_lower]
    ...
    data = [scout_id, match_num, team_num, a_color, on_field, left_tarmac, balls_auto, auto_upper, 
